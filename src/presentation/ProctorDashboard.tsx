/**
 * ProctorDashboard — presentation layer
 */
import React, { useState, useEffect, useRef } from 'react';
import type { StudentStatus } from '@shared/student';
import type { Session as UserSession } from '@shared/session';
import type { RecordingListItem } from '@shared/recording';
...
const STATUS_CONFIG: Record<StudentStatus, { label: string; color: string; bg: string }> = {
  ...
  cleared: { label: 'Оправдан',      color: '#22C55E', bg: '#052E16' },
};
...
export default function ProctorDashboard({ session }: ProctorDashboardProps) {
  const { username } = session;

  const { students, connectedCount } = useProctorSocket(username);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [commentText, setCommentText]   = useState('');
  const [tab, setTab] = useState<'students' | 'recordings' | 'chat'>('students');
  ...
  function setVerdict(verdict: StudentStatus) {
