...
import { useProctoring } from '../application/hooks/useProctoring';
import { useRecording } from '../application/hooks/useRecording';
import { useExamSocket } from '../application/hooks/useExamSocket';
import { useTabGuard } from '../application/hooks/useTabGuard';
...
type ExamPhase = 'setup' | 'exam' | 'banned' | 'done';
...
export default function StudentExam({ session, onLogout }: StudentExamProps) {
  ...
  } = useProctoring({ studentId: studentId! });

  const {
    recording, recordingHash,
    requestCamera, startRecording, stopRecording,
  } = useRecording({ studentId: studentId! });
  ...
  const { violations, score, autoScore } = useExamSocket({
    studentId: studentId!, username, onBanned: handleBanned,
  });

  useTabGuard({ studentId: studentId!, active: phase === 'exam' });
