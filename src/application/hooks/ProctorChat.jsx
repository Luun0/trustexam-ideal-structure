/**
 * ProctorChat — chat panel for proctor dashboard
 * Shows list of students who wrote, lets proctor reply
 */

import React, { useState, useEffect, useRef } from 'react';
import { socket } from '../../infrastructure/socketClient';

export default function ProctorChat({ proctorName }) {
  const [chats, setChats]         = useState({}); // { studentId: { name, messages[] } }
  const [activeId, setActiveId]   = useState(null);
  const [input, setInput]         = useState('');
  const [unreadMap, setUnreadMap] = useState({}); // { studentId: count }
  const bottomRef                 = useRef(null);

  useEffect(() => {
    const handler = (msg) => {
      if (msg.role !== 'student') return; // только от студентов
      const sid = msg.from;
      setChats(prev => {
